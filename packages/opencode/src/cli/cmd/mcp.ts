    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in carthis.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [path.join(baseDir, "carthis.json"), path.join(baseDir, "carthis.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".opencode", "carthis.json"), path.join(baseDir, ".opencode", "carthis.jsonc"))
  }
  ...
  // Default to carthis.json if none exist
  return candidates[0]
}
